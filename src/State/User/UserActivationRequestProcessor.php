 ...
 * @codeCoverageIgnore
 */
readonly class UserActivationRequestProcessor implements ProcessorInterface
{
    public function __construct(
        ...
        #[Autowire(service: 'limiter.register_activation_ip')]
        private RateLimiterFactory $ipLimiter,
        #[Autowire(service: 'limiter.register_activation_email')]
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): void
    {
        ...
        $lock = $this->lockFactory->createLock('register-activation:' . sha1($email), 30.0); // TTL 30s
        if (!$lock->acquire()) {
            return;
        }
        ...
        try {
            ...
            $this->logger?->error('register_activation_request_failed', ['email' => $email, 'ex' => $e]);
        } finally {
            $lock->release();
        }
    }
